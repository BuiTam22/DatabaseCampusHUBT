...
-- 4. Communication Feature Tables
...
CREATE NONCLUSTERED INDEX [idx_blog_author] ON [dbo].[Blog]([author_id]);
CREATE NONCLUSTERED INDEX [idx_message_participants] ON [dbo].[Messages]([sender_id], [receiver_id]);
CREATE NONCLUSTERED INDEX [idx_submission_status] ON [dbo].[Submission]([status], [user_id]);
...
AS
BEGIN
    SELECT u.*, ur.role, os.status
    FROM [dbo].[User] u
    LEFT JOIN [dbo].[User_Role] ur ON u.user_id = ur.user_id
    LEFT JOIN [dbo].[User_Status] os ON u.user_id = os.user_id
